...
def get_disk_usage(path='/'):
    """Asks the OS for the storage capacity of the given path."""
    try:
        st = os.statvfs(path)
        total = st.f_blocks * st.f_frsize
        # f_bavail is the free blocks available to non-root users
...
def draw_capacity_bar(stdscr, y, x, label, pct, details_str=""):
    """Draws a flexible progress bar. If details_str is provided, it prints that next to the %."""
...
def draw_dashboard(stdscr):
    ...
    while True:
        ...
        try:
            ...
            stdscr.addstr(1, 49, "Press 'q' to quit", curses.A_DIM)
            ...
            # --- CAPACITY BARS ---
            # CPU has no "Gigabytes", so we just pass the percentage
            draw_capacity_bar(stdscr, 8, 2, "CPU", cpu_pct)
            
            # RAM expects KB from /proc/meminfo, convert to GB
            ram_str = f"{ram_used/(1024**2):.1f} GB / {ram_total/(1024**2):.1f} GB"
            draw_capacity_bar(stdscr, 10, 2, "RAM", ram_pct, ram_str)
            
            # Disk expects Bytes from os.statvfs, convert to GB
            disk_str = f"{disk_used/(1024**3):.1f} GB / {disk_total/(1024**3):.1f} GB"
            draw_capacity_bar(stdscr, 12, 2, "DISK", disk_pct, disk_str)

            # --- TOP PROCESSES ---
            stdscr.addstr(15, 2, "TOP 5 PROCESSES (By Memory):", curses.color_pair(1) | curses.A_BOLD)
            stdscr.addstr(16, 2, f"{'PID':<8} {'RAM (MB)':<12} {'COMMAND'}", curses.A_UNDERLINE)
            
            row = 17
            for p in top_procs:
                cmd = p['cmd'][:45] + "..." if len(p['cmd']) > 45 else p['cmd']
                stdscr.addstr(row, 2, f"{p['pid']:<8} {p['ram_mb']:<12.1f} {cmd}")
                row += 1
